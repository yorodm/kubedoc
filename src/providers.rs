...
) -> anyhow::Result<rig_core::providers::openai::completion::CompletionModel> {
    let api_key_env = config
        .llm
        .api_key_env
        .as_deref()
        .unwrap_or("OPENAI_API_KEY");
    let api_key = std::env::var(api_key_env).map_err(|_| anyhow::anyhow!("{api_key_env} not set"))?;
...
pub fn anthropic_completion(
    config: &KubedocConfig,
) -> anyhow::Result<rig_core::providers::anthropic::completion::CompletionModel> {
    let api_key_env = config
        .llm
        .api_key_env
        .as_deref()
...
pub fn groq_completion(
    config: &KubedocConfig,
) -> anyhow::Result<rig_core::providers::groq::CompletionModel> {
...
pub fn ollama_completion(
    config: &KubedocConfig,
) -> anyhow::Result<rig_core::providers::ollama::CompletionModel> {
