...
pub fn openai_completion(
    ...
    let api_key_env = config.llm.api_key_env.as_deref().unwrap_or("OPENAI_API_KEY");
    let api_key = std::env::var(api_key_env)
        .map_err(|_| format!("{api_key_env} not set"))?;
...
pub fn anthropic_completion(
    ...
    let api_key_env = config.llm.api_key_env.as_deref().unwrap_or("ANTHROPIC_API_KEY");
    let api_key = std::env::var(api_key_env)
        .map_err(|_| format!("{api_key_env} not set"))?;
...
pub fn groq_completion(
    ...
    let mut builder = rig_core::providers::groq::Client::builder()
        .api_key(&api_key);
